/* ============================================================
   main.cjs -- COUNTY LINE as a desktop application.

   CommonJS, and named .cjs on purpose: the repository declares
   "type": "module" so that the test harnesses can import the game's own
   ES modules directly, and Electron's main process is not an ES module.

   Carried over from Final Rental essentially intact, and it is the piece
   of that repository that most deserved to be. It solves a real problem
   properly: a downloaded folder cannot load ES modules over file://, and
   the usual answers are either to give up modules or to run an HTTP
   server inside a shipped game. This does neither -- it registers a
   privileged scheme and serves the game to itself over game://app, so
   there is a real origin (relative imports, localStorage and pointer lock
   all need one), no listening socket, no port to collide with and no
   firewall prompt on first launch.

   The changes are the application's identity, the list of paths it will
