/* ============================================================
   ...
   Carried over from Final Rental unchanged apart from the additions at
   the foot of the file. It has no game in it at all: matrices, angles,
   and a seeded RNG.
...
/* ---- additions for County Line ---- */

export const clamp01 = (v) => (v < 0 ? 0 : v > 1 ? 1 : v);

/** Frame-rate independent exponential approach. `rate` is roughly
    "how many e-foldings per second", and the result never overshoots. */
export const approach = (a, b, rate, dt) => a + (b - a) * (1 - Math.exp(-rate * dt));

/** Move `a` toward `b` by at most `maxStep`. */
export const towards = (a, b, maxStep) =>
  Math.abs(b - a) <= maxStep ? b : a + Math.sign(b - a) * maxStep;

/** Position, yaw and a uniform scale -- the common case for a prop that
    is not authored at world size. */
export function setPosYawScale(m, x, y, z, yaw, s) {
  const c = Math.cos(yaw) * s, sn = Math.sin(yaw) * s;
  m[0] = c; m[1] = 0; m[2] = sn; m[3] = x;
  m[4] = 0; m[5] = s; m[6] = 0; m[7] = y;
  m[8] = -sn; m[9] = 0; m[10] = c; m[11] = z;
  return m;
}
